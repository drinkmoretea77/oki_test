    ...
    <div class="project-object panel panel-default">
        <div class="panel-heading">
            <h3 class="panel-title">
                {{ title }}&nbsp;<span v-if="items.length" class="badge badge-info">{{ items.length }}</span>
                <a href="javascript:" class="btn btn-xs blue btn-outline pull-right edit_type" data-loading @click="change">
        ...
        <div class="panel-body">
            ...
            <div v-if="!edit" class="row">
                <template v-if="items && items.length">
                    <div v-for="item in items" :key="item.id" class="col-lg-3 col-md-4 col-sm-6"><a :href="getUrl(name, item.id)" :data-name="name">{{ item.name }}</a></div>
                </template>
                <template v-else>
            ...
            <div v-else class="panel-edit-block">
                <div style="display: flex">
                    <v-search class="margin-bottom-15 margin-right-15" :placeholder="$t('Поиск')" @submit="updateSearchItems"/>
                        ...
                        >
                            <template #selected="{ option }">
                                <template v-if="option">
                                    <i v-if="option.color"
                                       class="fa fa-circle"
                                       :style="{ 'color': option.color, opacity: 0.6 }"
                                    ></i>
                                    {{ option.text }}
                                </template>
                            </template>
                ...
                <div class="row">
                    <div v-for="item in localItems" :key="item.id" v-show="!item.hide" class="col-lg-3 col-md-4 col-sm-6">
                        <div
                        ...
                        >
                            <div class="project-object__item-name">{{ item.name }}</div>
                            <template v-if="item.group">
                                <i class="fa fa-circle" :style="{color: item.group.color, opacity: 0.6}"></i>
                            </template>
                        </div>
                    </div>
                </div>
        ...
        methods: {
            ...
            updateHideItems(): void {
                for (const item of this.localItems) {
                    item.hide =
                        ((this.filterGroup !== -1) && item.group && (item.group.id !== this.filterGroup))
                        ||
                        ((this.searchRequest !== '') && !this.searchFilter(item.name));
                }
            },
            ...
            async change(event: any): Promise<any> {
                ...
                const response = await this.$api.get(
                    ...
                    {id: this.project.id, name: this.name},
                    {silent: true}
                );
                this.loading = false;
            ...
            async save(): Promise<any> {
                ...
                const selectedItems = this.localItems.filter(item => item.selected).map(item => item.id);
                ...
                if (data) {
                    ...
                    if (data.items) {
                        this.$store.dispatch('project/updateItems', {
                            name: this.name,
                            items: data.items
                        });
                    }
