...
var okitoki = {
    ...
    data_url: 'wss://' + location.host + ':8333/ws_proxy?dest=erl',
    ...
    phone_url: 'wss://' + location.host + ':8333/ws_proxy?dest=de2',
    ...
    data_init: function () {
        ...
        this.data_ws.onclose = function (evt) {
            console.log("data_websocket_closed:", evt.code);
    ...
    get_url: function (ip) {
        ...
        switch (ip) {
            case '193.19.229.145':
                ...
                return "wss://" + location.host + ":8333/ws_proxy?dest=ua1";
            case '195.201.13.68':
                //return "wss://lvp2-de.oki-toki.net:4014/";
                return "wss://" + location.host + ":8333/ws_proxy?dest=de2";
            case '185.100.65.25':
                //return "wss://lvp1-kz.oki-toki.net:4014/";
                return "wss://" + location.host + ":8333/ws_proxy?dest=kz1";
            case '144.76.104.216':
                //return "wss://lvp3-de.oki-toki.net:4014/";
                return "wss://" + location.host + ":8333/ws_proxy?dest=de3";
            default:
                //return "wss://lvp1-de.oki-toki.net:4014/";
                return "wss://" + location.host + ":8333/ws_proxy?dest=de1";
        }
    },
    ...
    phone_onmessage: function (data) {
        ...
        for (var i = 0; i < items.length; i++) {
            ...
            try {
                var json = JSON.parse(items[i]);
            } catch (e) {
                console.error(e);
                console.log(items[i]);
                return;
            }
            this.process_phone_json(json);
        }
    },

    process_phone_json: function (json) {
    ...
    data_onmessage: function (data) {
        if (data === 'ping') {
            ...
            // data = data.replace(/[\S\s]*<root>/gi, "<root>");
            // data = data.replace(/<\/root>[\s\S]*/gi, "</root>");
            // data = data.replace(/&/g, '&amp;');
            // var xmlDoc = $.parseXML(data);
            // var json = {};
            // $(xmlDoc).find('root').children().each(function () {
            //     json[this.tagName] = $(this).text();
            // });
            var items = data.split("\n");
            for (var i = 0; i < items.length; i++) {
                if (items[i].length === 0) {
                ...
                this.process_data_json(json);
            }
        }
    },

    process_data_json: function (json) {
        switch (json.type) {
            case 'user_online':
                this.fire_event('user_online', json);
                if (parseInt(json.user_from) === this.user_id) {
            ...
            case 'user_offline':
                ...
                break;
            case 'mm_status':
                this.fire_event('phone_status_changed', json);
                break;
            ...
            if (open) {
                ...
                $.ajax({
                    ...
                    if (okitoki.settings.pop_anketa.value) {
                        if (response.url.search('anketa.php') > -1) {
    ...
    is_action_disabled: function (button) {
        return !!(this.current_queue > 0 && this.queues[this.current_queue].disabled_buttons &&
    ...
    redirect: function (json) {
        ...
        switch (json.type) {
            case 'user':
                ...
                data = "action=redirect_user&redirect_user=" + json.user_id + "&session_id=" + json.session_id;
                break;
            case 'queue':
                command = {
                    cmgs: "queue_redirect|" + json.session_id + "|" + json.queue_id
                };
                data = "action=redirect_queue&redirect_queue=" + json.queue_id + "&session_id=" + json.session_id;
                break;
            case 'phone':
                var cid = 0;
                ...
                data = "action=redirect_phone&redirect_phone=" + json.phone + "&session_id=" + json.session_id + "&contact_id="+cid;
                break;
        }
    ...
    start_hold: function (json) {
        ...
        */
        var data = "action=start_hold&session_id=" + json.session_id;
    ...
    stop_hold: function (json) {
        ...
        */
        var data = "action=stop_hold&session_id=" + json.session_id;
        //this.phone_send(command); //commented due to ajax
        this.make_ajax(data);
    },
    ...
    intercept: function (json) {
        ...
        */
        var data = "action=intercept&session_id=" + json.session_id;
        //this.phone_send(command); //commented due to ajax
        this.make_ajax(data);
    },
    ...
    client2conf: function (json) {
        ...
        var data = "action=put_in_conf&session_id=" + json.session_id + "&conf_session_id=" + json.conf_session_id;
        //this.phone_send(command); //commented due to ajax
        this.make_ajax(data);
    },
    ...
    start_conf: function (json) {
        ...
        */
        var data = "action=start_conf&&session_id=" + json.session_id;
        //this.phone_send(command); //commented due to ajax
        this.make_ajax(data);
        return true;
    },
